...
func (s *stacks) Pop(column, count int) []string {
	elements := s.Queue[column][len(s.Queue[column])-count : len(s.Queue[column])]
	s.Queue[column] = s.Queue[column][:len(s.Queue[column])-count]
	return elements
}
...
func main() {
	...
	for _, v := range commands {
		elem := stackQueue.Pop(v.from, v.move)
