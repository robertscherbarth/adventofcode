...
func (i *ID) InRange(upper, lower int) bool {
	...
	if i.LowerLimit >= lower && i.LowerLimit <= upper {
		isInRange = true
	}

	if lower >= i.LowerLimit && lower <= i.UpperLimit {
		isInRange = true
	}

	if i.UpperLimit >= lower && i.UpperLimit <= upper {
		isInRange = true
	}

	if upper >= i.LowerLimit && upper <= i.UpperLimit {
		isInRange = true
	}
